...
function loadFormSubmitListener(form, modalBody) {
  ...
  function loadAddEmployeeModal() {
    fetch('/payroll-system/html/add_employee_html.php', {
      headers: {
        'X-Requested-With': 'XMLHttpRequest'
      }
    })
...
if (payrollSelect) {
    payrollSelect.addEventListener("change", () => {
        let payrollId = parseInt(payrollSelect.value);
        if (!isNaN(payrollId)) {
            govToggle.checked = (payrollId % 2 === 0); // ✅ even = checked
            govToggle.disabled = true; // ✅ lock it
        } else {
            govToggle.checked = false;
            govToggle.disabled = true; // always locked
        }
        updateTotal();
    });

    // run once on load in case payroll_id is already preselected
    let initialId = parseInt(payrollSelect.value);
    if (!isNaN(initialId)) {
        govToggle.checked = (initialId % 2 === 0);
        govToggle.disabled = true; // lock on load
    } else {
        govToggle.checked = false;
        govToggle.disabled = true;
    }
}
